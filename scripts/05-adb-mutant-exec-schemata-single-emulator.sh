...
# 
# Aeges
# 05-adb-mutant-exec-schemata-single-emulator.sh /home/auri/feup/tools/Projects/android/Aegis-schemata assemble app/build/outputs/apk/debug/app-debug.apk assembleAndroidTest app/build/outputs/apk/androidTest/debug/app-debug-androidTest.apk ids.txt random.txt test-case-names.txt com.beemdevelopment.aegis.debug.test/com.beemdevelopment.aegis.AegisTestRunner fullFail Pixel_6_Pro_API_30 5 1 10
# 
# AntennaPod
# 05-adb-mutant-exec-schemata-single-emulator.sh /home/auri/feup/tools/Projects/android/AntennaPod-schemata assembleFreeDebug app/build/outputs/apk/free/debug/app-free-debug.apk assembleAndroidTest app/build/outputs/apk/androidTest/free/debug/app-free-debug-androidTest.apk ids.txt random.txt test-case-names.txt de.danoeh.antennapod.core.tests/androidx.test.runner.AndroidJUnitRunner fullFail Pixel_6_Pro_API_30 5 1 10
#

if [ "$#" -ne 14 ]
then
  echo "Usage: 05-adb-mutant-exec-schemata-single-emulator.sh <project directory> <app build task> <app apk> <android test build task> <android test apk> <mutation id file list> <random file number list> <test case list> <package/test executor> <fastFail|fullFail> <emulator> <timeout in minutes> <from> <to>"
  exit 1
else
	# Script parameters
	PRJ=$1
